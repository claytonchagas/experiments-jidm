...
if __name__ == '__main__':
    ...
    temp_global_PARTICLE_SPIN = PARTICLE_SPIN
    PARTICLE_SPIN = 0.5
    spin = PARTICLE_SPIN
    PARTICLE_SPIN = temp_global_PARTICLE_SPIN
    ...
    temp_val = numpy.degrees(ab)
    abdeg = numpy.round(temp_val / ANGLE_RESOLUTION) * ANGLE_RESOLUTION

    ANGLE_RESOLUTION = 3.75
    temp_val = numpy.degrees(alice[:, 0])
    adeg = numpy.round(temp_val / ANGLE_RESOLUTION) * ANGLE_RESOLUTION

    ANGLE_RESOLUTION = 3.75
    temp_val = numpy.degrees(bob[:, 0])
    bdeg = numpy.round(temp_val / ANGLE_RESOLUTION) * ANGLE_RESOLUTION
    ...
    a = numpy.radians(angles)
    if spin == 0.5:
        temp_qm_curve = -numpy.cos(a)
    else:
        temp_qm_curve = numpy.cos(2 * a)
    ax1.plot(angles, temp_qm_curve, 'b-+', label='QM', lw=0.5)
